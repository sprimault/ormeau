...
func TestExtraireLesCasTordus(t *testing.T) {
	...
	// ALWAYS et BY DEFAULT se distinguent : l'une refuse une valeur explicite,
	// l'autre l'accepte, et Doctrine ne recrée que la seconde.
	t.Run("identite", func(t *testing.T) {
		if c := colonneOuEchouer(t, p, "t_client", "cli_id"); !c.AutoIncrement || c.Identite != calque.IdentiteToujours {
			t.Errorf("GENERATED ALWAYS AS IDENTITY : auto_increment %v, identite %q", c.AutoIncrement, c.Identite)
		}
		if c := colonneOuEchouer(t, p, "t_tag", "tag_id"); !c.AutoIncrement || c.Identite != calque.IdentiteParDefaut {
			t.Errorf("GENERATED BY DEFAULT AS IDENTITY : auto_increment %v, identite %q", c.AutoIncrement, c.Identite)
		}
	})
