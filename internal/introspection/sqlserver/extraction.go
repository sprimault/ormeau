...
func (p *pilote) lireColonnes(ctx context.Context, schema string, jeu *jeuDeTables) error {
	...
	for lignes.Next() {
		...
		var identite bool
		var defaut, calcul, collation, commentaire, sequenceSchema, sequenceNom, nomDefaut sql.NullString
		...
		if err := lignes.Scan(&table, &c.Nom, &c.Position, &typeSysteme,
			...
			&calcul, &persistee, &collation, &commentaire, &sequenceSchema, &sequenceNom, &nomDefaut); err != nil {
