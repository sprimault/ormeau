...
the bundle and `DatabaseDriver` left with ORM 3. Nothing official remains, and the
ecosystem alternatives are either abandoned or too naive for real legacy — they
produce `$clientId` as an `integer` instead of an association.

## Three layers, one pivot format
...
The **layer** — *calque* in French, the project's own vocabulary — is the pivot
format: the data model, serialized as JSON, describing a database with no
reference to the source DBMS nor to the target framework. The name comes from
tracing paper — a faithful copy of the structure — and from the linguistic sense:
a calque is a structural borrowing from one language into another, which is
...
The separation rests on an asymmetry: the physical level is an **observation**, it
can only be wrong through a bug; the logical level is a **judgement**, it can be
argued with. Mixing them would forbid any offline correction and make the tests
...
A pure function: no network, no side effects, no clock.
...
database` must yield an empty structural diff. What is not captured is lost — no
...
It does not speak Doctrine, but it speaks the vocabulary of the Hibernate family:
entities, associations with an owning and an inverse side, inheritance
strategies, composite identifiers. Doctrine, Hibernate, EF Core and TypeORM share
that model; GORM and Prisma do not — those consume the physical layer instead.
...
and no system dependency: a single binary you run on a customer's server without
...
Schema published in `schemas/`. As a consequence: inference tests are reference
files with no database, anyone can write an Eloquent or EF Core generator without
touching the core, and either half is independently rewritable.
...
`version_ri` is an integer, not semver: the only question that matters is whether
a generator can read the file. Adding an optional field does not change the
version; everything else increments it — renaming, removal, change of meaning, a
new value in a closed vocabulary. The generator refuses a version higher than the
one it knows.

The `empreinte` field has two distinct roles: in `source` it identifies a database
state; in the logical layer, `empreinte_physique` says which observation this
judgement derives from — which is how "the database moved since the last
generation" is detected without re-reading the entities.
